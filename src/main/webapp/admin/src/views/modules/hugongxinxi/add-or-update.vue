        ...
        <div v-else>
          <el-form-item v-if="ruleForm.zhaopian" label="照片" prop="zhaopian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.zhaopian.split(',')" :src="item" width="100" height="100">
                                                                                ...
                                                                                <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="护工介绍" prop="hugongjieshao">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="护工介绍"
                  v-model="ruleForm.hugongjieshao" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.hugongjieshao" label="护工介绍" prop="hugongjieshao">
                    <span>{{ruleForm.hugongjieshao}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xingbie = true;
            continue;
	            	            ...
	            	            if(o=='hugongjieshao'){
            this.ruleForm.hugongjieshao = obj[o];
	    this.ro.hugongjieshao = true;
            continue;
          }
	            	            if(o=='fuwujiage'){
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `hugongxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.hugongxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
